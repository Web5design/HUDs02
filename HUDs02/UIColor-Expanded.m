...
+ (CGFloat)urand
{
	return (CGFloat)(arc4random() % 4097) / 4096.0f;
}

+ (UIColor *)randomColor {
	return [UIColor colorWithHue:[UIColor urand] saturation:0.800f brightness:1.000f alpha:1.000f];
}
...
- (UIColor *)colorByAddingHue:(CGFloat)hue saturation:(CGFloat)sat brightness:(CGFloat)bri
{
	CGFloat oh, os, ob, oa;
	
	[self getHue:&oh saturation:&os brightness:&ob alpha:&oa];
	
	oh = MIN(1.0f, MAX(0.0f, oh + hue));
	os = MIN(1.0f, MAX(0.0f, os + sat));
	ob = MIN(1.0f, MAX(0.0f, ob + bri));
	
	return [UIColor colorWithHue:oh saturation:os brightness:ob alpha:oa];
}
